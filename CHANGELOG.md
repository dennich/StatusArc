...
### Changed
- Energy Mode rows now apply Automatic, Low Power, and supported High Power modes directly through a narrowly scoped, code-signature-validated helper. First use registers a previously unseen bundled helper correctly, macOS requests administrator approval once, and the helper uses a version handshake so authorization survives normally signed StatusArc updates.
- Energy Mode changes use macOS's unified Power Mode setting on current systems, with the earlier low/high-power settings retained as a compatibility fallback.
...
- The selected island now remains one persistent glass surface while its frame and corner radius expand or collapse. A critically damped spring drives the transition, neighboring islands use the native materialize transition, and panel sizing follows the same state without stale-value clipping. Energy Mode rows use native hover feedback, and the bolt/plug accessories sit 2 points farther right.
- Each island header now has one stable full-width click target during glass transitions; the Wi-Fi switch remains an independent native control.
